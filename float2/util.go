...
func ComponentsOf(v uint64, E, M uint64) []*big.Int {
	...
	if e == 0 && m == 0 {
		...
		return []*big.Int{
			new(big.Int).SetUint64(s),
			new(big.Int).SetUint64(e + M),
			new(big.Int).SetUint64(m + (1 << M)),
			new(big.Int).SetUint64(0),
		}
	} else {
		return []*big.Int{
...
func Read1DFile(path string) []uint32 {
	file, _ := os.Open(path)
	defer file.Close()
	...
	for scanner.Scan() {
		data, _ := strconv.ParseUint(scanner.Text(), 16, 32)
		ret = append(ret, uint32(data))
	}
	return ret
}

func Read2DFile(path string) [][]uint32 {
	file, _ := os.Open(path)
	defer file.Close()
	...
	for scanner.Scan() {
		...
		for i := 0; i < len(data); i++ {
			val, _ := strconv.ParseUint(data[i], 16, 32)
